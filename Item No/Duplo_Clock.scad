...
DUPLO_DOWN_POOL(X,Y,Z+1,X_2,Y_2,Z_2);
...
DUPLO_DOWN_POOL(X,Y,Z+2,X_2,Y_2,Z_2);//počet čudlíku x,y,Výčka z souřadnice zobrazení x,y,z;    
} 
    
    DUPLO_CIRCLE(X,Y,Z,1,0,0,1);
}

DUPLO_CIRCLE(X,Y,Z,1,0,1,1);
}